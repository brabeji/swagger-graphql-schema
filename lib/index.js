	...
	(0, _traverse2.default)(rootSchema).forEach(function parseObjectType(schema, context) {
		...
		if (isPlainType && !isCached) {
			...
			typesCache[schemaId] = new _graphql.GraphQLObjectType({
				...
				fields: () => Object.keys(properties).filter(propertyName => propertyName !== discriminatorFieldName).reduce((acc, propertyName) => {
					const propertySchema = properties[propertyName];
					let type = scalarTypeFromSchema(propertySchema);
					if (!type) {
						type = typesCache[propertySchema.$$type];
...
const constructInputType = ({ schema, typeName: inputTypeName, typesCache, isNestedUnderEntity = false, discriminatorFieldName }) => {
	...
	if ((0, _lodash.isArray)(schema.anyOf)) {
		inputType = new _graphqlUnionInputType2.default({
			name: typeName,
			types: schema.anyOf.map(unionPartSchema => constructInputType({
	...
	if (!inputType) {
		...
		const a = Object.keys(properties).filter(k => !properties[k].readOnly && !properties[k][IS_IN_INPUT_TYPE_CHAIN_SYMBOL]);
