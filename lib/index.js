	...
	if ((0, _lodash.isArray)(valueType)) {
		...
		switch (valueType.toLowerCase()) {
			...
			case 'object':
				...
				break;
			case 'string':
				const enumValues = (0, _lodash.get)(schema, 'enum');
				if (enumValues) {
					const newType = new _graphql.GraphQLEnumType({
						name: typeName,
						values: enumValues.reduce((acc, enumValue, i) => {
...
const swaggerToSchema = (swagger, idFormats = ['uniqueId', 'uuid']) => {
	...
	const schema = new _graphql.GraphQLSchema(Object.assign({
